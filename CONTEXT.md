...
**Elector Limit**:
The administrator-configured maximum number of non-withdrawn ballot records that may be accepted for one election. Each election has its own limit, every non-withdrawn group sequence is confined to the range from 1 through this limit, and the limit cannot be reduced below either the active record count or the highest active group sequence.
...
**Ballot Record**:
A recording group's digital representation of one physical ballot for one election. Every record has a permanent record identifier, while only a non-withdrawn record occupies a group sequence and ballot number.
...
**Group Sequence**:
A reusable positive integer allocated from the smallest number not currently occupied by a recording group's non-withdrawn ballot records. It is bounded by the election's elector limit and returns only to the same group's pool after withdrawal.
_Avoid_: Global sequence number, record identifier

**Ballot Number**:
The current display identity of a non-withdrawn ballot record: the Union Committee Election combines the recording group's ordinal and group sequence as `group-sequence`, while the single-group Expense Review Committee Election uses the group sequence alone. It is unique among an election's non-withdrawn records.
_Avoid_: Record identifier, permanent sequence number, global sequence number

**Withdrawn Ballot Record**:
A previously submitted ballot record that its recording group has removed from the tally because it was entered incorrectly. It remains independently identifiable and retains its former group-sequence assignment as audit data, but has no current ballot number, contributes no totals, and has no designated replacement record; its former sequence returns to the same recording group's pool.
